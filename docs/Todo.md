...
7) [해결] 클릭시 TempDragger 리셋 안됨 버그 & ECS 누를시 TempDragger 리셋
8) [해결] grid 사이에 불필요한 빈공간생김 해결
9) [해결] 포스트잇 디자인 레이아웃 및 디자인 개선 - 메인 색상 연동, 깔끔한 아이콘으로 변경
10) [해결] 포스트잇 pointerDown stopPropagation
11) 배경화면 변경 버튼 지금보다 작게, 왼쪽 하단에 붙어있으면서 오른쪽에 숨기기 토글 버튼 있음.
12) [해결] 포스트잇 상단 제목 입력 기능 추가 (가운데 정렬)
 - [해결] 포스트잇 접기/펼치기 기능 추가 (애니메이션 포함)
...
16) (크리티컬 버그) 최초 창 크기가 전체 화면이 아닐 때 배치가 달라지게됨.
17) [해결] 포스트잇 클릭하면 앞쪽으로 나오게하기
18) 버그 - 북마크 검색기 검색결과 안보임(살짝 보임). 아마 내가 max-height 로 된 걸 height 로 바꿔서 그런 것 같은데.
19) 폴더매니저가 포스트잇들 보다 먼저 보이게하기.
20) multi drag 이동후에 초기화 되지 않게 하기. 클릭이벤트에만 초기화되게하기.
21) 포스트잇 화면 변화에 반응하지 않게하기?? -> 이건 좀 애매함.
22) [버그] 삭제된 북마크가 있던 위치로 옮겨지지 않음.
